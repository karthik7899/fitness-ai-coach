...
These files in `core/src/main/resources/aura/` come from the Python app. They
are the places where two implementations would otherwise drift apart:
...
Shared files in `fixtures/` pin behaviour the same way:

- import behaviour;
- the assembled prompt (`prompt_assembly.json`);
- the grounding check's verdicts (`grounding.json`).

`EvalTest` runs the eval scenarios in `evals/scenarios/` through this coach
with the model scripted, the same files the Python suite runs through its own.
See `evals/README.md`.
